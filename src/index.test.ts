...
describe('findFastestEndpoints', () => {
  ...
  describe('region deduplication', () => {
    it('deduplicates endpoints by region keeping only one per region', async () => {
      const endpoints: EndpointConfig[] = [
        { url: 'https://region-a-1.xyz', region: 'region-a', type: 'direct' },
        { url: 'https://region-a-2.xyz', region: 'region-a', type: 'cloudflare' },
        { url: 'https://region-b.xyz', region: 'region-b', type: 'direct' },
      ];
      ...
      const regionAResults = results.filter(r => r.region === 'region-a');
      expect(regionAResults.length).toBe(1);
    });

    it('keeps only fastest endpoint per region', async () => {
      const endpoints: EndpointConfig[] = [
        { url: 'https://region-a-slow.xyz', region: 'region-a', type: 'cloudflare' },
        { url: 'https://region-a-fast.xyz', region: 'region-a', type: 'direct' },
      ];

      // We need to ensure consistent ordering - since timing can be unpredictable,
      // we verify that only one region-a endpoint is returned
      mockFetch.mockImplementation((url: string) => {
        if (url.includes('github') || url.includes('raw.githubusercontent')) {
          return Promise.reject(new Error('Skip remote'));
        }
      ...
      // Should only have one entry for region-a
      const regionACount = results.filter(r => r.region === 'region-a').length;
      expect(regionACount).toBe(1);
    });

    it('handles all endpoints in same region', async () => {
      const endpoints: EndpointConfig[] = [
        { url: 'https://same-1.xyz', region: 'same-region', type: 'direct' },
        { url: 'https://same-2.xyz', region: 'same-region', type: 'cloudflare' },
        { url: 'https://same-3.xyz', region: 'same-region', type: 'direct' },
      ];
      ...
      // Should only return 1 endpoint since all are same region
      expect(results.length).toBe(1);
      expect(results[0].region).toBe('same-region');
    });

    it('deduplication works correctly with includeAutoRouted', async () => {
      const endpoints: EndpointConfig[] = [
        { url: 'https://region-a-1.xyz', region: 'region-a', type: 'direct' },
        { url: 'https://region-a-2.xyz', region: 'region-a', type: 'cloudflare' },
      ...
      // Should have 1 region-a endpoint + auto endpoint
      expect(results.filter(r => r.region === 'region-a').length).toBe(1);
      expect(results.filter(r => r.region === 'auto').length).toBe(1);
      expect(results[results.length - 1].region).toBe('auto'); // Auto should be last
    });
  });
